...
duplicate_blocks (basic_block *bb_list, int bb_count)
{
  ...
  /* Joey added. If is not within a loop, don't duplicate it.
     This is to avoid a crash in gimple_duplicate_sese_region. */
  //if (orig_edge && orig_edge->dest && orig_edge->dest->loop_father)
    //{
    bool ret =
    gimple_duplicate_sese_region (orig_edge, exit_edge, &bb_list[1], bb_count-2,
				  NULL, false);
    
      if (dump_file)
        {
	  fprintf(dump_file, "gimple_duplicate_sese_region %d result: %s\n",
		  orig_edge->dest->index,
	          ret ? "pass":"fail");
        }
    //}
  //else
    //fprintf(stderr, "%d: Found unavailable edge to duplicate\n", ++i);
